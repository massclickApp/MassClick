...
const InterlockBricksCards = () => {
  ...
  const interlockBricks = businessList.filter((b) =>
    b.businessName?.toLowerCase().includes("Interlock Bricks".toLowerCase())
);

    if (interlockBricks.length === 0) {
        return <p>No matching businesses found with the name "Fencing".</p>;
    }
