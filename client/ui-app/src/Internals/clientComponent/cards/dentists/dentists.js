...
const DentistsCards = () => {
    ...
    const dentists = businessList.filter((b) =>
        b.businessName?.toLowerCase().includes("Dentists".toLowerCase())
    );

    if (dentists.length === 0) {
        return <p>No matching businesses found with the name "Dentists".</p>;
    }
