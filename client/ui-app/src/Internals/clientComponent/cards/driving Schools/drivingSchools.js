...
const DrivingSchoolsCards = () => {
  ...
  const drivingSchools = businessList.filter((b) =>
    b.businessName?.toLowerCase().includes("Driving Schools".toLowerCase())
);

    if (drivingSchools.length === 0) {
        return <p>No matching businesses found with the name "Driving Schools".</p>;
    }
